...
//Swift 提供兩個運算子：is跟as來檢查一個值的型別以及將一個值轉換成另一種型別。
//基於類別繼承的特性，一個子類別繼承於另一個父類別時，這個子類別的型別除了可以是自己之外，也可以是父類別型別。下面例子以三個類別(一個基礎類別及兩個繼承自它的子類別)示範is及as的使用方法：
...
//根據上面建立的類別，接著宣告一個陣列常數，包含 2 個Archer實體及 1 個Warrior實體，而初始化時會自動推斷出Archer跟Warrior有一個共同的父類別GameCharacter，所以這個陣列會自動推斷型別為[GameCharacter]，如下：
...
//型別檢查
//
//使用型別檢查運算子is來檢查一個實體是否屬於一個類別。檢查後會返回一個布林值，是這個類別會返回true，反之則返回false。 以下使用for-in來遍歷前面宣告的陣列gameTeam：
// 用來計算弓箭手跟戰士的數量
...
//你也可以用is來檢查一個類別是否遵循了某個協定(protocol)。
//後面章節會正式介紹協定。

//向下型別轉換
//
//一個類別的常數或變數，可能實際上是屬於一個子類別(像是前面提的例子)。當遇到這種情況，可以嘗試使用型別轉換運算子as?或as!將其向下型別轉換至子類別型別。
//如果不確定向下型別轉換是否能夠成功，則使用as?來轉換，這會返回一個可選值，如果向下型別轉換失敗會返回一個nil，這樣的特性可以讓你檢查向下型別轉換是否成功。
//如果確定向下型別轉換一定成功時，則可以使用強制性的as!來轉換。但如果轉換至一個錯誤的型別，則會觸發程式運行時錯誤。
//以下仍用前面宣告的陣列gameTeam來做例子：
...
//Hint
//
//為了型別安全，我們應該明確地指定值或實體的型別，除非是真的必要或確切需要才使用Any跟AnyObject。
//AnyObject
//
//這邊使用前面定義的類別Archer做例子：
