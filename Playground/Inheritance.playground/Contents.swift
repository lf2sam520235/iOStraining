...
//使用關鍵字override來表示你要覆寫這個特性(即方法、屬性或下標)。
//覆寫方法
//
//以下為一個覆寫方法的例子：
// 使用並改寫前面定義的類別 Hunter
...
//上述程式中可以知道，willSet觸發時，會先執行子類別的再來才是父類別的，而didSet則是相反，先執行父類別的再來才是子類別的。

//存取父類別的屬性,方法及下標
//
//在前面章節介紹類別的時候，提到類別有一個隱藏的內建屬性 self，可以在方法中代表類別本身。而當繼承自另一個類別時，可以使用super屬性來存取父類別的屬性、方法或下標。
//方法someMethod()的覆寫實作裡，使用super.someMethod()來呼叫父類別的someMethod()方法。
//屬性someProperty的getter及setter覆寫實作裡，使用super.someProperty來存取父類別的someProperty屬性。
//下標的覆寫實作裡，使用super[someIndex]來存取父類別的下標。
//以下為一個例子：
// 使用並改寫前面定義的類別 Hunter
...
//防止覆寫
//
//可以在類別的方法、屬性或下標前面加上final，來防止它們被覆寫，使用方式為final var、final func或是final class func這樣。
//甚至也可以在整個類別的關鍵字class前面加上final，這樣整個類別都不能再被繼承。
